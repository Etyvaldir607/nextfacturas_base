...
if ($id_proforma == 0) {
	...
	foreach ($proformas as $nro => $proforma) {
		$body .= '<tr>';
		$body .= '<td>' . ($nro + 1) . '</td>';
		$body .= '<td>' . escape(date_decode($proforma['fecha_egreso'], $_institution['formato']) . ' ' . $proforma['hora_egreso']) . '</td>';
		$body .= '<td>' . escape($proforma['nombre_cliente']) . '</td>';
		$body .= '<td>' . escape($proforma['nit_ci']) . '</td>';
		$body .= '<td align="right">' . escape($proforma['nro_factura']) . '</td>';
		$body .= '<td align="right">' . escape(number_format($proforma['monto_total'], 2, ',', '.')) . '</td>';
...
EOD;
	...
	// Titulo del documento
	$pdf->Cell(0, 10, 'PREVENTA # ' . escape($proforma['nro_factura']), 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	$valor_fecha = escape(date_decode($proforma['fecha_egreso'], $_institution['formato']) . ' ' . $proforma['hora_egreso']);
	$valor_nombre_cliente = escape($proforma['nombre_cliente']);
	$valor_nit_ci = escape($proforma['nit_ci']);
	$valor_nro_proforma = escape($proforma['nro_factura']);
...
EOD;
	...
	// Genera el nombre del archivo
	$nombre = 'prreventa_' . $id_proforma . '_' . date('Y-m-d_H-i-s') . '.pdf';
}
