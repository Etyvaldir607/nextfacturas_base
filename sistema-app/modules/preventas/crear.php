			...
			<div class="panel-body">
				...
				<div class="col-sm-4 hidden-xs  text-right">
					<div class="form-check form-check-inline">
						<label class="form-check-label" for="inlineCheckbox1">Búsqueda de Productos</label>
	...
	<?php if ($id_almacen) { ?>
		<div class="col-md-6">
			<div class="panel panel-success">
				...
				<div class="panel-body">
					<h2 class="lead text-success">Preventa : <?= escape($almacen['almacen']); ?></h2>
					<hr>
					<form id="formulario" class="form-horizontal">
						<div style="zoom: 1;">
							...
							<div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <label class="radio-inline"> <input type="radio" name="facturar" id="sin_factura" value="0" onchange="facturar_datos(false)" checked> Sin factura </label>
                                    <label class="radio-inline"> <input type="radio" name="facturar" id="con_factura" value="1" onchange="facturar_datos(true)" > Con factura </label>
                                </div>
							</div>
							<div class="form-group">
							...
							<!-- Joseama:: add  -->
							<!-- <div class="form-group" id="para_ciudad">
								<label for="ciudad" class="col-sm-4 control-label">Ciudad:</label>
								<div class="col-sm-8">
									<select name="ciudad" id="ciudad" class="form-control" data-validation="required" data-validation-allowing="-+./&()">
									<?php 
										// foreach($Datos as $Fila=>$Dato):
									?>
										<option value='<?php //$Dato['id_ciudad']?>'><?php //"{$Dato['ciudad']} ({$Dato['departamento']})"?></option>
									<?php
										// endforeach;
									?>
									</select>
							...
							<div class="form-group" id="para_tipo">
								<label for="tipo" class="col-sm-4 control-label">Tipo:</label>
								<div class="col-sm-8">
									<select name="tipo_cli" id="tipo_cli" class="form-control" data-validation="required" data-validation-allowing="-+./&()">
										<?php // $tipos = $db->select('*')->from('inv_tipos_clientes')->fetch();
										//foreach ($tipos as $nro => $tipo) { ?>
											<option value="<?php // $tipo['tipo_cliente'] ?>"><?php // $tipo['tipo_cliente'] ?></option>
										<?php //} ?>
									</select>
								</div>
							</div> -->
							...
							<!--				<option value="<?php // escape($ruta['id_ruta']); ?>"><?php // escape($ruta['nombre']); ?></option>-->
							<!--			<?php //} ?>-->
							<!--		</select>-->
							<!--	</div>-->
							<!--</div>-->
										...
										<div class="col-sm-4">
											<select name="prioridad" id="prioridad" class="form-control text-uppercase" data-validation="required" data-validation-allowing="-+./&()">
												...
												<?php foreach ($prioridades as $prioridad) { ?>
													<option value="<?= escape($prioridad['prioridad']); ?>" <?= ($prioridad['prioridad'] == 'ALTA' || $prioridad['prioridad'] == 'Alta') ? 'selected' : '' ?> ><?= escape($prioridad['prioridad']); ?></option>
												<?php } ?>
											</select>
										</div>
										<div class="col-sm-8">
							...
							<table id="ventas" class="table table-bordered table-condensed table-striped table-hover margin-none">
								<thead>
									<tr class="active">
										...
										<?php if($_user['rol_id'] == 1 || $_user['rol_id'] == 2) { ?>
								...
								<tfoot>
									<tr class="active">
										<th class="text-nowrap text-right" colspan="<?= ($_user['rol_id'] == 1 || $_user['rol_id'] == 2) ? 6 : 6 ?>">IMPORTE TOTAL <?= escape($moneda); ?></th>
										...
										<tr class="active cuotaclass">
											...
											<td>
												<div data-cuota="<?= $i ?>" class="cuota_div">
													<div class="col-sm-12">
														<input id="inicial_fecha_<?= $i ?>" name="fecha[]" value="<?= ($i == 1)?date('d-m-Y'):'' ?>" min="<?= date('Y-m-d') ?>" class="form-control input-sm" autocomplete="off" data-validation-format="DD-MM-YYYY" data-validation-min-date="<?= date('d-m-Y'); ?>" onchange="javascript:change_date(<?= $i ?>);" onblur="javascript:change_date(<?= $i ?>);">
													</div>
												</div>
											</td>
											<td>
						...
						<div class="form-group">
							<div class="col-xs-12 text-right">
								...
								<button type="submit" class="btn btn-success" onmouseup="SetGuardar(event);">Guardar</button>
	...
	<div class="modal-dialog">
		<form method="post" id="form_asignar" class="modal-content loader-wrapper" autocomplete="off">
			...
			<div class="modal-footer">
				<!--                    <input type="checkbox" class="form-check-input" id="materialChecked2" name="prof" id="prof">-->
				<button type="button" onclick="enviar_form()" class="btn btn-primary">
...
<script>
	...
	$(function() {
		...
		$cliente.selectize({
			...
			$('#tipo').val(0);
			if (valor.length != 1) { //Antiguo Cliente
				...
				facturar_datos(false);
				// console.log('vendeor: ' + $vendedor_id);
				...
				// $('#para_tipo').css({'display':'none'});
				// $('#para_ciudad').css({'display':'none'});
				// Josema::add
			} else { //Nuevo Cliente
				...
				// Josema::add
				// $('#para_tipo').css({'display':'block'});
				// $('#para_ciudad').css({'display':'block'});
				// Josema::add
			}
		...
		$.validate({
			...
			onSuccess: function() {
				// guardar_proforma();
				if(SwGuardar==true){
                    bootbox.confirm('Está seguro que guardar la preventa?', function (result) {
    					if(result){
    						guardar_proforma();
    					}
    					else{
				...
				$.ajax({
					...
					if (productos.length) {
						...
						for (var i in productos) {
							...
							$ultimo.eq(9).attr('data-contado', productos[i].id_producto);
	...
	function adicionar_producto(id_producto) {
		...
		var rol = <?= ($_user['rol_id'] == 1 || $_user['rol_id'] == 2) ? 1 : 0 ?>;
		...
		if (porciones.length > 2) {
			...
			for (var ic = 1; ic < porciones.length; ic++) {
				...
				plantilla = plantilla + '<option value="' + oparte[1] + '" data-pr="'+ id_producto +'" data-xyyz="' + id_producto + '" data-yyy="' + parte[1] + '" data-yyz="' + porci2[ic - 1] + '" data-asig="' + asignation[ic - 1] + '" >' + oparte[1] + ' ('+parte[1]+')</option>';
			}
			plantilla = plantilla + '</select></td>' +
				'<td class="text-middle">' +
				'<input type="text" value="' + aparte[1] + '" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="' + aparte[1] + '" data-validation-error-msg="Debe ser un número decimal positivo" onkeypress="SetEnter(event);" onkeyup="calcular_importe(\'' + id_producto + '\')">' +
				'<input type="text" value="' + aparte[1] + '" name="pre[]" class="form-control input-xs text-right hidden" autocomplete="off" data-pre="' + aparte[1] + '" data-validation-error-msg="Debe ser un número decimal positivo" onkeypress="SetEnter(event);" onkeyup="calcular_importe(\'' + id_producto + '\')">' +
			...
			plantilla = plantilla + '<td class="text-middle">' +
				'<input type="text" value="' + parte[0] + '" data-xyyz="' + id_producto + '" name="unidad[]" class="form-control input-xs text-right" autocomplete="off" data-unidad="' + parte[0] + '" readonly data-validation-error-msg="Debe ser un número decimal positivo"></td>' +
				
				'<td class="text-middle" data-xyyz="' + id_producto + '" >';
	            ...
	            ?>			
        			if(editarxxx!='S') {
        				...
        				plantilla = plantilla + '<input type="text" value="' + parte[1] + '" name="precios[]" class="form-control input-xs text-right" autocomplete="off"  data-precio="' + parte[1] + '" data-cant2="1" data-validation-error-msg="Debe ser un número decimal positivo" onkeypress="SetEnter(event);" onkeyup="modificar_precio(\'' + id_producto + '\'); calcular_importe(\'' + id_producto + '\')">' +
        				                        '<input type="text" value="' + parte[1] + '" name="pre[]" class="form-control input-xs text-right hidden" autocomplete="off"  data-pre="' + parte[1] + '" data-cant2="1" data-validation-error-msg="Debe ser un número decimal positivo" onkeypress="SetEnter(event);" onkeyup="modificar_precio(\'' + id_producto + '\'); calcular_importe(\'' + id_producto + '\')">';
        			}
				<?php
                }else{
                ?>			
		...
		plantilla = plantilla + '<td class="text-middle hidden">'+
    ...
    function calcular_importe(id_producto) {
		...
		$.ajax({
			...
			success: function( data ){
    			...
    			//alert(precioedit+" + "+data['precio_mayor']+" + "+id_producto+" +++ "+id_producto_simple);
	...
	function guardar_proforma() {
    		...
    		$.ajax({
    			...
    			if (proforma) {
    				...
    				// // 	imprimir_recibo(proforma.id_egreso);
    				// } else {
...
// 	}
    function imprimir_recibo(nota) {
		...
		setTimeout(imprimir_nota(nota),3000);
	}
	...
	function set_cuotas() {
		...
		for (i = 1; i <= cantidad; i++) {
			...
			$('#inicial_fecha_'+ i).attr('data-validation','required date');
		}
		for (i = parseInt(cantidad) + 1; i <= 36; i++) {
			...
			$('#inicial_fecha_'+ i).removeAttr('data-validation');
		}
		set_cuotas_val();
		calcular_cuota(1000);
	}
	...
	/////////////////////////////////////////////////////////////////////
	function facturar_datos(condi){
	    if(condi == true){
	        $('#sin_factura').prop('checked', false);
	        $('#con_factura').prop('checked', true);
	        $('#nit_ci').prop('readonly', false);
	        $('#nombre_cliente').prop('readonly', false);
	    } else {
	        $('#sin_factura').prop('checked', true);
	        $('#con_factura').prop('checked', false);
	        $('#nit_ci').prop('readonly', true);
	        $('#nombre_cliente').prop('readonly', true);
	    }
	} 
