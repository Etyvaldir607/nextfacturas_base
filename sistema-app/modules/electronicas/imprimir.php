...
if ($id_egreso == 0) {
	// Obtiene las egresos
	$egresos = $db->select('p.*, f.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')
					->from('inv_egresos p')
	...
	// Obtiene la egreso
	$egreso = $db->select('p.*, f.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')
				  ->from('inv_egresos p')
				  ->join('inv_egresos_facturas f', 'f.egreso_id = p.id_egreso', 'left')
				  ->join('inv_almacenes a', 'p.almacen_id = a.id_almacen', 'left')
				  ->join('sys_empleados e', 'p.empleado_id = e.id_empleado', 'left')
...
if ($id_egreso == 0) {
	...
	foreach ($detalles as $nro => $detalle) {
		...
		$body .= '<td class="left-right">' . escape($detalle['nombre_factura']).' <br>(LOTE: '. escape($detalle['lote']) .' VENC: '. escape($detalle['vencimiento']) .')' . '</td>';
