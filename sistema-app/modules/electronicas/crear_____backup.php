...
$clientes = $db->query("SELECT *
						...
						LEFT JOIN sys_users u ON persona_id=cg.vendedor_id
						LEFT JOIN sys_supervisor ss ON u.id_user=ss.user_id
...
$productos = $db->query("SELECT p.id_producto, p.asignacion_rol, p.descuento ,p.promocion,
						...
						LEFT JOIN (
							SELECT d.producto_id, SUM(IF(e.tipo = 'Preventa' && e.preventa = 'habilitado', d.cantidad, IF(e.tipo='No venta' && e.estadoe = 4, d.cantidad, IF(e.tipo NOT IN ('Preventa', 'No venta'), d.cantidad, 0)))) AS cantidad_egresos, d.lote
...
$empleados = $db->query("SELECT CONCAT(e.nombres, ' ', e.paterno, ' ', e.materno) as empleado, e.id_empleado, r.rol, u.id_user, nombre_grupo as codigo
						...
						left join inv_clientes_grupos on VENDEDOR_ID=e.id_empleado
		...
		<div class="panel panel-success">
			<div class="panel-heading">
				<h3 class="panel-title">
					...
					<strong>Venta electrónica</strong>
				</h3>
			</div>
	...
	<?php if ($almacen) { ?>
		<div class="col-md-6">
			<div class="panel panel-info">
				...
				<div class="panel-body">
					<h2 class="lead text-warning">Venta electrónica: <?= escape($almacen['almacen']); ?></h2>
					<hr>
					<form id="formulario" class="form-horizontal">
						...
						<!-- <div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" value="" name="telefono" id="telefono" class="form-control text-uppercase" autocomplete="off" data-validation="required" data-validation-allowing="-+./&() " data-validation-length="max100">
							</div>
						</div> -->
						...
						<!-- <div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" value="" name="ubicacion" id="ubicacion" class="form-control text-uppercase" autocomplete="off" data-validation="required">
							</div>
						</div> -->
						<div class="form-group">
			...
			<div class="panel panel-info" data-servidor="<?= ip_local . name_project . '/nota.php'; ?>">
				<div class="panel-heading">
					<h3 class="panel-title">
						<span class="glyphicon glyphicon-menu-hamburger"></span>
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<?php if ($permiso_mostrar) : ?>
						<p class="text-right">
							<a href="?/electronicas/mostrar" class="btn btn-primary">Mis ventas</a>
						</p>
					<?php endif ?>
	...
	<?php } else { ?>
		<div class="col-xs-12">
			<div class="panel panel-success">
				...
				<div class="panel-body">
					<div class="alert alert-danger">
						<p>Usted no puede realizar la venta electronica, verifique que la siguiente información sea correcta:</p>
						<ul>
...
<h2 class="btn-warning position-left-bottom display-table btn-circle margin-all display-table" data-toggle="tooltip" data-title="Esto es una venta electronica" data-placement="right">
...
<!-- <table class="hidden">
	<tbody id="fila_filtrar" data-negativo="<?= imgs; ?>/" data-positivo="<?= files; ?>/productos/">
		<tr>
			...
			<td class="hidden" data-contado=""></td>
		</tr>
		<tr>
					...
					if (productos.length) {
						...
						for (var i in productos) {
							...
							if ((parseInt(productos[i].cantidad_ingresos) - parseInt(productos[i].cantidad_egresos)) > 0) {
								...
								// $ultimo.eq(2).find('span').attr('data-vencimientoView', productos[i].id_producto);
								// $ultimo.eq(2).find('span').text(productos[i].vencimiento);
								// $ultimo.eq(2).find('span').attr('data-vencimiento', productos[i].id_producto);
								// $ultimo.eq(2).find('span').text(productos[i].vencimiento);
								// $ultimo.eq(2).find('span').attr('data-lote', productos[i].id_producto);
								// $ultimo.eq(2).find('span').text(productos[i].lote);
	...
	function adicionar_producto(id_producto) {
		...
		if (porciones.length > 2) {
			...
			plantilla = plantilla + '</select></td>' +
				...
				'<input type="text" value="' + aparte[1] + '" readonly name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="' + aparte[1] + '"   data-validation-error-msg="Debe ser un número decimal positivo" onkeypress="SetEnter(event);" onkeyup="calcular_importe(\'' + id_producto + '\')">' +
    			...
    			if (result) {
            		...
            		$.ajax({
            			...
            			success: function( result ){
            				console.log(result);
            				$.notify({
            					message: 'La venta electronica fue realizada satisfactoriamente.'
            				}, {
            					type: 'success',
            					delay: 50000,
            					timer: 60000,
            				...
            				if($('#distribuir').val()=="N"){
                				if(result.recibo == 'si'){
                				    imprimir_recibo(result.egreso_id);
                				    // $.open('?/electronicas/aux_imprimir_nota/' + result.egreso_id, true); //marcador
                				    //intentar imprimir el documentoq ue falta desde ajax
                				    
                				    // $.open('?/notas/recibo_dinero/' + result.egreso_id, true);
                				} else {
            			...
            			error: function( error ){
            				...
            				$('#loader').fadeOut(100);
            				$.notify({
            					message: 'Ocurrió un problema en el proceso, no se puedo guardar los datos de la venta electronica, verifique si la se guardó parcialmente.'
            				}, {
            					type: 'danger'
            				});
            			}
...
// 		window.open('?/electronicas/recibo_dinero/' + nota, true);
// 		setTimeout(imprimir_nota(nota),3000);
// 	}
	
// 	function imprimir_nota(nota) {
// 		$.open('?/electronicas/imprimir/' + nota, true);
// 		window.location.reload();
// 	}
    
//     function imprimir_recibo(nota) {
// 		window.open('?/electronicas/imprimir_todo/' + nota, true);
//         window.location.reload();
// 	}
	...
	function imprimir_nota(nota) {
		bootbox.confirm('¿Desea imprimir la Factura?', function(result) {
			if (result) {
		        $.open('?/electronicas/imprimir/' + nota, true);
		        window.location.reload();
			}
