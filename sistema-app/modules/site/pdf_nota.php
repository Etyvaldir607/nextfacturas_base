...
if ($id_egreso == 0) {
	// Obtiene las egresos
	$egresos =   $db->select('p.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno, c.cliente, v.nombres nombresv, v.paterno paternov, v.materno maternov,nombre_grupo')
					->from('inv_egresos p')
	...
	// Obtiene la egreso
	$egreso =  $db->select('p.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno, c.cliente, c.direccion, c.telefono, v.nombres nombresv, v.paterno paternov, v.materno maternov,nombre_grupo')
				  ->from('inv_egresos p')
				  ->join('inv_almacenes a', 'p.almacen_id = a.id_almacen', 'left')
				  ->join('sys_empleados e', 'p.empleado_id = e.id_empleado', 'left')
				  ->join('sys_empleados v', 'p.vendedor_id = v.id_empleado', 'left')
