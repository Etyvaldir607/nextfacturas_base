...
foreach($clientes as $cliente2){
    $t_clientes = $t_clientes.'*'.$cliente2['ubicacion'];
    $n_clientes = $n_clientes.'*'.$cliente2['cliente'];

    $aux = explode('|',$cliente2['empresa']);
            ...
            <form method="post" id="cliente_form" action="?/clientes/guardar" class="form-horizontal" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        ...
                        ?>
                            <option value='<?=$Dato['id_ciudad']?>' <?php if($Dato['id_ciudad']==$cliente['ciudad_id']):echo 'selected';endif;?>><?="{$Dato['ciudad']} ({$Dato['departamento']})"?></option>
                        <?php
                            endforeach;
                        ?>
                        </select>
                    ...
                    <div class="col-sm-9">
                        <select name="id_grupo" id="id_grupo" class="form-control">
                            ...
                            foreach ($grupos as $nro => $grupo) {
                                if($cliente['cliente_grupo_id']==$grupo['id_cliente_grupo']){ ?>
                                    <option value="<?= $grupo['id_cliente_grupo'] ?>" selected><?= $grupo['nombre_grupo'] .' - ('. $grupo['vendedor'] . ')'?></option>
                                    <?php }else{?>
                ...
                <div class="form-group">
                    <label for="tipovv" class="col-sm-3 control-label">Semana:</label>
                    <div class="col-sm-9">
                        <select name="id_dia" id="id_dia" class="form-control">
                            <option value=""></option>
                            <option value="1" <?php if($cliente['dia']=='1'){ echo "selected"; } ?> >Lunes</option>
                            <option value="2" <?php if($cliente['dia']=='2'){ echo "selected"; } ?> >Martes</option>
                            <option value="3" <?php if($cliente['dia']=='3'){ echo "selected"; } ?> >Miercoles</option>
                            <option value="4" <?php if($cliente['dia']=='4'){ echo "selected"; } ?> >Jueves</option>
                            <option value="5" <?php if($cliente['dia']=='5'){ echo "selected"; } ?> >Viernes</option>
                            <option value="6" <?php if($cliente['dia']=='6'){ echo "selected"; } ?> >Sabado</option>
                        </select>
                    </div>
                </div>
                <!-- <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <select name="empleado" id="empleado" class="form-control text-uppercase" data-validation="required" data-validation-allowing="-+./&()">
                            ...
                            <?php // foreach ($empleados as $empleado) { ?>
                                <option value="<?php // escape($empleado['id_empleado']); ?>" <?php // ($empleado['id_empleado'] == $cliente['empleado_id']) ? 'selected' : ''?>  ><?php // escape($empleado['empleado']); ?></option>
                            <?php // } ?>
                        </select>
                    </div>
                </div> -->
            ...
            if(validaForm()){
                ...
                $.ajax({ //datos que se envian a traves de ajax
                    ...
                    if (ruta.estado == 's') {
            			...
            			window.location = '?/clientes/listar';
