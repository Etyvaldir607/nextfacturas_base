    ...
    <?php if ($permiso_imprimir) { ?>
        <div class="row">
            <div class="col-sm-6 hidden-xs">
                <div class="text-label">Para ver el reporte hacer clic en el siguiente bot贸n: </div>
            </div>
        ...
        <thead>
            <tr class="active">
                ...
                <?php if ($permiso_modificar || $permiso_eliminar) : ?>
                    <th class="text-nowrap">Dia de Preventa</th>
                <?php endif ?>
            </tr>
        </thead>
        <tfoot>
            <tr class="active">
                ...
                <th class="text-nowrap" data-datafilter-filter="true">Direcci贸n</th>
                <th class="text-nowrap" data-datafilter-filter="true">Tipo</th>
                <th class="text-nowrap" data-datafilter-filter="true">Grupo</th>
                <?php if ($permiso_modificar || $permiso_eliminar) : ?>
                    <th class="text-nowrap text-middle" data-datafilter-filter="false">Dia de Preventa</th>
                <?php endif ?>
            </tr>
        </tfoot>
        <tbody>
        ...
        var table = $('#table').DataFilter({
            ...
            values: {
                ...
                createdRow: function(nRow, aData, iDisplayIndex) {
                    ...
                    $('td', nRow).eq(8).addClass('');
                    <?php
                    if ($permiso_modificar || $permiso_eliminar) :
                    ?>
                        $('td', nRow).eq(9).addClass('');
                    <?php
                    endif;
                    ?>
                }
...
function guardar_dia(id_cliente){
    dia=$("#dia_"+id_cliente).val();
    $.ajax({
		url: '?/clientes/guardar_dia',
		type: 'post',
		data: "id="+id_cliente+"&dia="+dia,
