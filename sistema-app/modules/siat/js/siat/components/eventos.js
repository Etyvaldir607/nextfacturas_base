	...
	ns.ComEventos = {
							...
							<div class="form-group mb-3">
								<div class="row">
									<div class="col-12 col-sm-6">
										...
										<input type="date" name="" value="" class="form-control" ref="eventofechainicio" required />
										<div class="invalid-feedback">Necesita seleccionar una fecha de inicio del evento</div>
									</div>
									<div class="col-12 col-sm-6">
										<label>Hora Inicio</label>
										<input type="time" name="" value="" class="form-control" ref="eventohorainicio" required />
					...
					<form ref="formcerrar" action="" method="" class="modal-content" novalidate>
						...
						<div class="modal-body">
							<template v-if="eventoIndex > -1 && items[eventoIndex]">
								<siat-cerrar-evento ref="comcerrar" v-bind:evento="items[eventoIndex]" v-on:evento-cerrado="eventoCerrado" />
							</template>
						</div>
						<div class="modal-footer">
							<button type="button" class="btn btn-danger" data-dismiss="modal" data-coreui-dismiss="modal">Cerrar</button>
		...
		methods:
		{
			...
			async sync() {
				...
				const puntoventa = this.sync_puntoventa_id
				const date = this.sync_date
				try {
					this.$root.$processing.show('Obteniendo datos...');
					//const res = await this.$root.http.Get(`?/siat/api_sincronizaciones/sync_eventos/${sucursal}/${puntoventa}/${date}`);
					const res = await this.$root.http.Get(`?/siat/api_eventos/obtener_eventos/${sucursal}/${puntoventa}`);
					this.items = res.data || [];
			...
			async getTipoEventos() {
				const res = await this.$root.http.Get(`?/siat/api_sincronizaciones/sync_tipo_eventos`);
				//console.log(res.data.RespuestaListaParametricas.listaCodigos)
			...
			async getCufds() {
				...
				const res = await this.$root.http.Get(`?/siat/api_sincronizaciones/sync_cufd/${this.evento.sucursal_id}/${this.evento.puntoventa_id}`);
				this.cufds = (res.data) ? [res.data] : [];
			},
			async getItems(page, sucursal, puntoventa) {
				...
				try {
					this.$root.$processing.show('Obteniendo datos...');
					const res = await this.$root.http.Get(`?/siat/api_eventos/obtener_eventos/${sucursal}/${puntoventa}/${page}`);
					this.items = res.data;
					this.$root.$processing.hide();
				}
				catch (e) {
			...
			async guardar() {
				try {
					...
					const res = await this.$root.http.Post(`?/siat/api_eventos/crear_evento`, this.evento);
					this.$root.$processing.hide();
			...
			async validarRecepcion(item, index) {
				try {
					...
					//const res = await this.$root.api.Get(`/../sistema/?/siat/api_eventos/${item.id}/validar-recepcion`);
					const res = await this.$root.http.Get(`?/siat/api_eventos/validar_recepcion/${item.id}`);
					//console.log(res);
					this.$root.$processing.hide();
					this.filter();
				}
			...
			async anular(item, index) {
				try {
					...
					const res = await this.$root.http.Put(`?/siat/api_eventos/anular_evento`, { id: item.id });
					this.$root.$processing.hide();
					this.filter();
				}
				catch (e) {
