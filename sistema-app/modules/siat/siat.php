...
function siat_get_config()
{
	...
	$config = new SiatConfig([
		...
		/*'tokenDelegado'	=> 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9..F0U-oZwOK09tTpNzxXEDWhinV8viwQOvs9-ejghqB_zjAcVHPmM083d8I6XAw3k6PyxaVMSHXPJ3K4SxC-89Gg',*/
...
function siat_obtener_cufd($sucursal = 0, $puntoventa = 0, $date = null)
{
	...
	$query = sprintf("SELECT * FROM mb_siat_cufd WHERE sucursal_id = %d AND puntoventa_id = %d AND DATE(fecha_vigencia) >= '%s' ORDER BY id DESC LIMIT 1",
		$sucursal, $puntoventa, $cdate
	);
...
function siat_motivos_anulacion($sucursal, $puntoventa)
{
	echo $filename = sprintf("%s/%d-%d-motivos-anulacion.json", SIAT_DATA_DIR, $sucursal, $puntoventa);
	if( !siat_file_needs_sync($filename) )
		return json_decode(file_get_contents($filename));
	$servicio = siat_servicio_syncronizacion($sucursal, $puntoventa);
...
function siat_leyenda()
{
	$leyendas = siat_obtener_leyendas();
...
function siat_egreso2factura($egreso)
{
	...
	$facturaSiat->cabecera->cuf								= $egreso['cuf'] ?: null;
	...
	$facturaSiat->cabecera->fechaEmision					= date('Y-m-d\TH:i:s.v');
	//$facturaSiat->cabecera->fechaEmision					= date('Y-m-d\TH:i:s.v', /*strtotime($invoice->invoice_date_time)*/);
...
function siat_recepcion_factura($egreso_id)
{
	...
	$cufd 			= siat_obtener_cufd();
	
	if( !$cufd )
		$cufd = siat_renovar_cufd();
	if( !$cufd )
		throw new Exception('No se puede obtener un CUFD valido');
	$egreso['leyenda'] 					= siat_leyenda();
	$egreso['tipo_factura_documento'] 	= SiatInvoice::FACTURA_DERECHO_CREDITO_FISCAL;
	$eventoActivo = siat_eventos_obtener_activo((int)$egreso['codigo_sucursal'], (int)$egreso['punto_venta']);
	if( $eventoActivo )
	{
		...
		$facturaSiat->buildCuf(0, $config->modalidad, $egreso['tipo_emision'], $egreso['tipo_factura_documento'], $egreso['codigo_control']);
...
//var_dump($res->RespuestaServicioFacturacion);
		...
		if( $res->RespuestaServicioFacturacion->codigoEstado != 908 )
		{
			...
			throw new ExceptionInvalidInvoiceData('Ocurrio un error con la recepcion de la factura. SIAT: ' .$egreso['nro_factura']. "---" . \sb_siat_message($res->RespuestaServicioFacturacion),
				null,
				(object)$egreso
			);
		}
...
function siat_puntos_venta_db()
{
	...
	$query = 'SELECT * FROM mb_siat_puntos_venta ORDER BY id DESC';
	$items = $db->query($query)->fetch();
...
function siat_eventos_cerrar(int $id)
{
	...
	$config	= siat_get_config();
	$cuis	= siat_obtener_cuis();
	$cufd 	= siat_obtener_cufd($localEvent->sucursal_id, $localEvent->puntoventa_id);
	//$config->validateExpirations();
	if( !siat_es_contingencia($localEvent->evento_id) )
		$localEvent->fecha_fin = date('Y-m-d H:i:s');
	//if( $localEvent->status == 'PROCESSING' )
	//	return;
		
	//$localEvent->status = 'PROCESSING';
	//$localEvent->Save(0);
	...
	if( $total <= 500 )
	{
		...
		//sb_update_user_meta($user->user_id, '_SIAT_'. $localEvent->puntoventa_id . '_MANUAL_EVENT', 0);
		siat_eventos_verificar($localEvent);
	}
	else
	{
		//##envio masivo
...
function siat_eventos_verificar(object $evento)
{
	...
	if( $res->RespuestaServicioFacturacion->codigoDescripcion == 'VALIDADA' )
		$db->where(['evento_id' => $evento->id])->Update('mb_invoices', [
...
function siat_eventos_db($sucursal, $puntoventa, $page = 1, $limit = 25)
{
	...
	$query = sprintf("SELECT * FROM mb_siat_eventos WHERE sucursal_id = %d AND puntoventa_id = %d ORDER BY id DESC LIMIT %d, %d",
		$sucursal,
		$puntoventa,
		$offset, 
		$limit
...
function siat_crear_puntoventa($sucursal, $tipo, $nombre)
{
	global $db;
	...
	$res = $servicio->registroPuntoVenta($sucursal, $tipo, $nombre);
...
{
	$config = siat_get_config();
	return sprintf("https://%s.impuestos.gob.bo/consulta/QR?nit=%d&cuf=%s&numero=%d&t=%d",
