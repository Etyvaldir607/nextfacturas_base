...
// Obtiene el user
$usuario = $db->from('sys_token')->where('user_id', $id_user)->fetch_first();

// Verifica si el user existe
if ($usuario && $usuario['id_user'] != 1) {
	// Elimina el user
	$db->delete()->from('sys_token')->where('user_id', $id_user)->limit(1)->execute();
	...
	$data = array(
		...
		'proceso' => 'u',
		'nivel' => 'l',
		'direccion' => '?/usuarios/eliminar_dispositivo',
	...
	if ($db->affected_rows) {
		...
		$_SESSION[temporary] = array(
			...
			'message' => 'El celular fue eliminado correctamente.'
		);
	}
