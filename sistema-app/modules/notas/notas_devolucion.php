<?php
    ...
    $detalles = $db->query("SELECT a.*, b.*,e.categoria, a.unidad_id AS unidad_det, d.unidad, SUM(a.cantidad)as cantidad
                            ...
                            GROUP BY a.vencimiento, a.lote, a.producto_id, a.precio
                            ")->fetch();
    ...
    $pagos_q = "SELECT p.id_pago, p.movimiento_id, p.tipo, SUM(IF(d.estado = 1 , d.monto, 0)) as pagado, 
                        e.descripcion as egreso, e.fecha_egreso, e.hora_egreso, e.nro_nota, id_egreso
                    FROM inv_egresos e
    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-sm-8 hidden-xs">
                <div class="text-label">Para volver al listado hacer clic en el siguiente botón: </div>
            </div>
                            ...
                            <form method="post" action="?/notas/notas_devolucion_guardar" class="form-horizontal">
                                    ...
                                    <div class="col-sm-8 col-sm-offset-4">
                                        <div class="form-check form-check-inline">
                                            <label class="form-check-label">
                                                <input class="form-check-input" type="radio" name="tipo" value="Devolucion" checked> Devolución
                                            </label>
                                        </div>
                                        <div class="form-check form-check-inline mt-3">
                                            <label class="form-check-label">
                                                <input class="form-check-input" type="radio" name="tipo" value="Reposicion" > Reposición
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    <div class="col-xs-12">
                                        ...
                                        <input type="text" name="monto_total" value="0" class="translate" tabindex="-1" data-total="" data-validation="required number" data-validation-allowing="range[0.00;1000000.00],float" data-validation-error-msg="El monto total de la venta debe ser mayor a cero y menor a 1000000.00">
                                    </div>
                                </div>
                                <hr>
                                    ...
                                    <table class="table table-bordered table-hover table-responsive table-sm" id="tabla_deudas" style="max-width: 100% !important;">
                                        ...
                                        <tbody class="text-sm">
                                            ...
                                            foreach ($pagos as $key => $pago) {
                                                ...
                                                $total_q = "SELECT SUM(dd.cantidad*dd.precio)as monto_total
                                                                FROM inv_egresos e
                                                                LEFT JOIN inv_egresos_detalles dd ON id_egreso=egreso_id
                                                                WHERE e.nro_nota='".$pago['nro_nota']."' 
                                                                ";
                                                $total = $db->query($total_q)->fetch_first();
                                                
                                                //echo " /////// ".$total['monto_total']." - ".$pago['pagado'];
                                                
                                                if($total['monto_total'] - $pago['pagado'] > 0){ ?>
                                                    <tr>
                                                        <td><?= date_decode($pago['fecha_egreso'], $_institution['formato']) . ' <br><small>' . $pago['hora_egreso'] ?></small></td>
                                                        <td><?= $pago['nro_nota'] ?></td>
                                                        
                                                        <td class="text-right"><?= number_format(($total['monto_total']), 2, ',', '.') ?></td>
                                                        <td class="text-right"><?= number_format(($pago['pagado']), 2, ',', '.') ?></td>
                                                        <td class="text-right danger"><?= number_format( ($total['monto_total']-$pago['pagado']), 2, ',', '.') ?></td>
                                                        
                                                        <td><?= ucwords(str_replace("Venta de productos con ", "", $pago['egreso'])) ?></td>
                                                        <td>
                                                            <input type="checkbox" id="para_pagar_<?= $Dato['id_detalle'] ?>" name="para_pagar[]" value="<?= $Dato['id_detalle'] ?>" >
                                                        </td>
                                                        <td>
                                                            <INPUT type="hidden" class="form-control" value="<?= $pago['id_egreso'] ?>" name="egreso_pagar[]" id="egreso_pagar<?= $pago['id_egreso'] ?>">
                                                            <INPUT type="text" class="form-control monto_pagar" value="0" name="monto_pagar[]" id="monto_pagar<?= $pago['id_egreso'] ?>" onchange="Acumulados();" onkeyup="Acumulados();" style="text-align:right;">
                                                        </td>
                                                    </tr>
                                                <?php  
                                                }
                                        ...
                                        <tbody class="text-sm">
                                            <tr>
                                                ...
                                                <td data-acumulados="" style="text-align:right;">
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                ...
                                <div class="form-group">
                                    <div class="col-xs-12 text-right">
                                        <button type="submit" class="btn btn-primary hidden" id="submit_button">
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div class="alert alert-danger">
                                ...
                                <ul>
                                    <li>No existe el almacén seleccionado de la cual se puedan tomar los productos necesarios para la venta, debe fijar un almacén.</li>
                                </ul>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function adicionar_producto(id_producto) {
        ...
        if ($producto.size()) {
            ...
            '<td class="text-nowrap text-middle"><input type="text" value="' + idproducto + '" name="productos[]" class="translate" tabindex="-1" data-validation="required number" data-validation-error-msg="Debe ser número">' + codigo + '</td>' +
            ...
            if(porciones.length>2){
                ...
                '<td class="text-middle"><input readonly type="text" value="' + aparte[1] + '" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="' + aparte[1] + '"  readonly  data-validation-error-msg="Debe ser un número decimal positivo" onkeyup="calcular_importe(' + id_producto + ')"></td>';
            }
            else{
                sincant = porciones[1].split(')');
                console.log(sincant);
                parte = sincant[1].split(':');
                plantilla = plantilla + '<td class="text-middle"><input type="text" value="' + parte[0] + '" name="unidad[]" class="form-control input-xs text-right" autocomplete="off" data-unidad="' + parte[0] + '" readonly data-validation-error-msg="Debe ser un número decimal positivo"></td>'+
                '<td class="text-middle"><input  readonly type="text" value="' + parte[1] + '" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="' + parte[1] + '"  data-validation-error-msg="Debe ser un número decimal positivo" onkeyup="calcular_importe(' + id_producto + ')"></td>';
            }
    ...
    function Acumulados(){
        var $ventas = $('#tabla_deudas tbody');
        var $total = $('[data-acumulados]:first');
        var $importes = $ventas.find('.monto_pagar');
        var importe, total = 0;
        ...
        var $total_1 = $('[data-subtotal]:first');
        total_1=$total_1.text();
        total_2f = parseFloat(total_1);
