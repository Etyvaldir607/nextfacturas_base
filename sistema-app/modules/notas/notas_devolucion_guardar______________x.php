    ...
    if (isset($_POST['id_egreso']) && isset($_POST['id_cliente']) && isset($_POST['nit_ci']) && isset($_POST['nombre_cliente']) && 
        ...
        if ($tipo == 'Reposicion') {
            ...
            foreach($productos as $nro => $producto){
                ...
                $verif_f = $db->select('factura, factura_v, IVA')
                              ->from('inv_ingresos_detalles')
                              ->where('producto_id', $productos[$nro])
                              ->where('lote', $lotes[$nro])
                              ->where('vencimiento', $vencimiento[$nro])
                              ->fetch_first();
        ...
        if ($tipo == 'Devolucion') {
            ...
            if ($egreso_modif['plan_de_pagos'] == 'si') {
                ...
                if ($total_salado) {
                    $total_saldo = $total_salado['saldo'] - $monto_total;
                } else {
                    $total_saldo = 0;
                }
