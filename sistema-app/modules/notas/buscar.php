	...
	if (isset($params)) {
		...
		if (isset($_POST['busqueda'])) {
			...
			// Obtiene los productos con el valor buscado
				// $productos = $db->query("SELECT p.id_producto, z.id_asignacion, z.unidad_id, z.unidade, z.cantidad2, p.descripcion, p.imagen, p.codigo, p.nombre, p.nombre_factura, p.cantidad_minima, p.precio_actual, IFNULL(e.cantidad_ingresos, 0) AS cantidad_ingresos, IFNULL(s.cantidad_egresos, 0) AS cantidad_egresos, u.unidad, u.sigla, c.categoria, e.vencimiento, e.lote,e.id_detalle
				...
				WHERE 	p.visible='s' AND
				        (p.codigo like '%{$busqueda}%' 
						OR p.nombre_factura like '%{$busqueda}%' 
