...
if (is_post()) {
    ...
    if (isset($_POST['id_egreso']) && isset($_POST['id_cliente']) && isset($_POST['nit_ci']) && isset($_POST['nombre_cliente']) && 
        ...
        foreach ($monto_pagar as $key222 => $para_pagarx) {
            ...
            $montox=($monto_pagar[$key222]!="")?$monto_pagar[$key222]:0;
        
            $query1="  SELECT SUM(i.monto)as saldo
                       from inv_pagos_detalles i 
                       inner join inv_pagos p ON p.id_pago=i.pago_id 
                       where i.estado=1 
                             AND movimiento_id=".$idx." 
                             AND p.tipo='Egreso'
                      ";
            
            $deuda_v = $db->query($query1)->fetch_first();
            
            $query2 = " SELECT SUM(cantidad*precio) as monto_total 
                        from inv_egresos i 
            ...
            $monto_total = $db->query($query2)->fetch_first();
            
            if ($deuda_v && $montox>0) {
                $saldo=$monto_total['monto_total']-$deuda_v['saldo'];
                    
                if ( $saldo<$montox ) {
                    set_notification('danger','La deuda es menor al monto que desea devolver! '.$saldo.' < '.$montox,'Por favor seleccione otra deuda para pagar con la devolucion.');
        ...
        redirect('?/notas/notas_devolucion/'.$_POST['id_egreso']);
