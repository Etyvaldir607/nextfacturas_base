...
$almacenes = $db->query("   SELECT *
                            FROM inv_almacenes 
                            ")->fetch();
		
foreach ($almacenes as $nro => $almacen) { 
    
    $alma1 = $almacen['id_almacen'];
    
    $productos_por_lote = $db->query("  SELECT producto_id, vencimiento, lote, costo, sum(lote_cantidad)as lote_cantidad, id_detalle
                                        FROM inv_ingresos_detalles d
                                        INNER JOIN inv_ingresos i ON id_ingreso=ingreso_id 
                                        where 
                                            
                                            producto_id>=31 and producto_id<=40 
                                            
                                            and i.almacen_id='".$alma1."'
                                        GROUP BY producto_id, vencimiento, lote, costo
                        				")->fetch();
    
    foreach ($productos_por_lote as $nro222 => $Dato) { 
    
        
        $costo1 = $Dato['costo'];
        $lo1 = $Dato['lote'];
        $ve1 = $Dato['vencimiento'];
        $pro1 = $Dato['producto_id'];
        
        $ingresos = $db->query("SELECT SUM(d.resultado) as cantidad
                                FROM vista_stock_costos d 
                                
                                WHERE d.almacen_id = '$alma1'
                                    AND d.producto_id = '$pro1'
                                    AND d.lote = '$lo1'
                                    AND d.vencimiento = '$ve1'
                                    AND (d.costo_producto = '$costo1' OR d.costo_producto IS NULL) 
                                ")->fetch_first();
                                
        if($ingresos['cantidad']!=$Dato['lote_cantidad'] || $ingresos['cantidad']<0){
            
            // echo $alma1." ".$pro1." ///// <b style='color:#f88;'>".$ingresos['cantidad']."</b> ".$Dato['lote_cantidad']." ///// ".$Dato['costo']." ".$Dato['lote']." ".$Dato['vencimiento']."<br>";                        

            //     $db->query("UPDATE inv_ingresos_detalles
            //                 SET lote_cantidad='0'
            ...
            if($ingresos['cantidad']>=0){
                $db->query("UPDATE inv_ingresos_detalles
                            SET lote_cantidad='0'
                            WHERE   id_detalle IN ( SELECT id_detalle
                                                    FROM inv_ingresos_detalles d
                                                    INNER JOIN inv_ingresos i ON id_ingreso=ingreso_id 
                                                    WHERE i.almacen_id = '$alma1'
                                                        AND d.producto_id = '$pro1'
                                                        AND d.lote = '$lo1'
                                                        AND d.vencimiento = '$ve1'
                                                        AND (d.costo = '$costo1' OR d.costo IS NULL) 
                                    				) 
                        ")->execute();
    
                $db->query("UPDATE inv_ingresos_detalles
                            SET lote_cantidad='".$ingresos['cantidad']."'
                            WHERE   id_detalle = '".$Dato['id_detalle']."' 
                        ")->execute();
            }
        }
        ...
        if($ingresos['cantidad']<0){
            ...
            echo $ingresos['cantidad']." ".$Dato['id_detalle'];

            

            echo "<span style='color:#ff0'>".$alma1." ".$pro1." ///// <b style='color:#f88;'>".$ingresos['cantidad']."</b> ".$Dato['lote_cantidad']." ///// ".$Dato['costo']." ".$Dato['lote']." ".$Dato['vencimiento']."</span><br>";                        
            $lotes_internos = $db->query("  SELECT producto_id, vencimiento, lote, costo, sum(lote_cantidad)as lote_cantidad, id_detalle
                                            FROM inv_ingresos_detalles d
                                            INNER JOIN inv_ingresos i ON id_ingreso=ingreso_id 
                                            where 
            ...
            foreach ($lotes_internos as $nro333 => $Dato3) { 
            
                
                $costo1 = $Dato3['costo'];
                $lo1 = $Dato3['lote'];
                $ve1 = $Dato3['vencimiento'];
                $pro1 = $Dato3['producto_id'];

                $ingresos_internos= $db->query("SELECT SUM(d.resultado) as cantidad
                                                FROM vista_stock_costos d 
                ...
                echo "<a href='?/bancos/listar/".$Dato['id_detalle']."/".$Dato3['id_detalle']."/".($ingresos['cantidad']*(-1) )."'>".$alma1." ".$pro1." ///// <b style='color:#afa;'>".$ingresos_internos['cantidad']."</b> ".$Dato['lote_cantidad']."  ///// ".$Dato3['costo']." ".$Dato3['lote']." ".$Dato3['vencimiento']."</a> <br>";                        
    ...
    //                     FROM inv_egresos 
    //                     LEFT JOIN gps_noventa_motivos m ON motivo_id=id_motivo
    //                 ")->fetch();
                        
    // foreach($Lotes as $Fila=>$Lote):
    //     $Condicion=[
    //             'id_egreso'=>$Lote['id_egreso'],
    //         ];
    //     $Datos=[
    //             'motivo_id'=>$Lote['motivo'],
    //         ];
    ...
    // $Lots=$db->query(" SELECT *
    //                     FROM inv_pagos_detalles 
    //                     WHERE fecha!='0000-00-00'
    //                 ")->fetch();
                        
    // foreach($Lots as $Fila=>$Lot):
    //     $Condicion=[
    //             ' 	id_pago_detalle'=>$Lot['id_pago_detalle'],
    //         ];
    //     $Datos=[
    //             'monto_programado'=>$Lot['monto'],
    //         ];
    //     $db->where($Condicion)->update('inv_pagos_detalles',$Datos);
    // endforeach;
