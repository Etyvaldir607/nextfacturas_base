<?php
    ...
    FROM (  SELECT *
            ...
            GROUP BY w.producto_id ) z ON p.id_producto = z.producto_id")->fetch();
    ...
    $pagos = $db->query("SELECT p.id_pago, p.movimiento_id, p.tipo, SUM(IF(d.estado = 1 , d.monto, 0)) as pagado, SUM(IF(d.estado = 0 , d.monto, 0)) as saldo, 
                                e.monto_total, e.descripcion as egreso, e.fecha_egreso, e.hora_egreso, e.nro_nota
                        FROM inv_pagos p
                        JOIN inv_pagos_detalles d ON p.id_pago = d.pago_id 
                        LEFT JOIN inv_egresos e ON p.movimiento_id = e.id_egreso
    ...
    <div class="panel-body">
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-4 text-right">
                <a href="?/operaciones/notas_listar" class="btn btn-primary"><span class="glyphicon glyphicon-arrow-left"></span><span class="hidden-xs"> Listar</span></a>
            </div>
        </div>
        <hr>
        <div class="row">
                ...
                <div class="col-md-6">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            ...
                            <?php } ?>
                            <form method="post" action="?/operaciones/notas_guardar" class="form-horizontal">
                                            ...
                                            foreach ($pagos as $key => $pago) {
                                                if($pago['monto_total'] - $pago['pagado'] > 0){ ?>
                                                    <tr>
                                                        ...
                                                        <td class="text-right danger"><?= number_format(($pago['saldo']), 2, ',', '.') ?></td>
                                                        <td><?= ucwords(str_replace("Venta de productos con ", "", $pago['egreso'])) ?></td>
                                                        <td><label class="radio-inline"><input type="radio" name="para_pagar" id="para_pagar<?= $pago['id_pago'] ?>" value="<?= $pago['id_pago'] ?>">Pagar</label></td>
                                                    </tr>
                                                <?php  
                                                }
                                            }   
                            ...
                            <?php if ($productos) { ?>
                                <table id="productos" class="table table-bordered table-condensed table-striped table-hover table-xs">
                                    ...
                                    <?php foreach ($detalles as $nro => $detalle) {
                                        ...
                                        $otro_precio = $db->select('max(costo) as costo')->from('inv_ingresos_detalles')->where('lote =', $detalle['lote'])->where('vencimiento =', $detalle['vencimiento'])->where('producto_id =', $detalle['id_producto'])->fetch_first();
                                        // $db->select('*')->from('inv_asignaciones a')->join('inv_unidades b','a.unidad_id=b.id_unidad')->where('a.producto_id',$detalle['id_producto'])->fetch();
    ...
    function adicionar_producto(id_producto) {
        ...
        if ($producto.size()) {
            ...
            if(porciones.length>2){
                ...
                '<td class="text-middle"><input type="text" value="' + aparte[1] + '" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="' + aparte[1] + '"  readonly  data-validation-error-msg="Debe ser un número decimal positivo" onkeyup="calcular_importe(' + id_producto + ')"></td>';
            }
            else{
                ...
                '<td class="text-middle"><input type="text" value="' + parte[1] + '" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="' + parte[1] + '"  data-validation-error-msg="Debe ser un número decimal positivo" onkeyup="calcular_importe(' + id_producto + ')"></td>';
            }
