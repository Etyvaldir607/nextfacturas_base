...
$entrega = (sizeof($params) > 0) ? $params[1] : 0;
...
$detalles = $db->query("SELECT a.*, b.*, a.unidad_id AS unidad_det, GROUP_CONCAT(c.unidad_id, '*',d.unidad, '*', c.otro_precio SEPARATOR '|') AS prec, e.categoria
...
<?php require_once show_template('header-empty'); ?>
        ...
        <div class="col-md-6">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <?php } ?>
                    <form method="post" action="?/asignacion/preventas_actualizar" class="form-horizontal">
                            ...
                            <table id="ventas" class="table table-bordered table-condensed table-striped table-hover table-xs margin-none">
                                ...
                                <?php foreach($detalles as $key => $detalle){
                                    ...
                                    <tr class="active" data-producto="<?= $detalle_id_producto; ?>">
                                        ...
                                        <td class="text-middle"><input type="text" value="<?= $detalle['cantidad'] ?>" name="cantidades[]"  class="form-control text-right input-xs" maxlength="10" autocomplete="off" 
                                            data-cantidad="" data-validation="required number" data-validation-allowing="range[1;<?= $det_ingreso['cantidad_lote'] ?>]" 
                                            data-validation-error-msg="Debe ser un número positivo entre 1 y <?= $det_ingreso['cantidad_lote'] ?>" 
                                        ...
                                        <?php } ?>
                                        <td class="text-middle"><input type="text" value="<?= $detalle['precio'] ?>" name="precios[]" class="form-control input-xs text-right" autocomplete="off" 
                                            data-precio="<?= $detalle['precio'] ?>"  data-validation-error-msg="Debe ser un número decimal positivo" 
                                            onkeyup="calcular_importe('<?= $detalle_id_producto ?>',1)" <?php if(!$promociones){ ?> readonly="readonly" <?php } ?> ></td>
                                        ...
                                        <td class="text-nowrap text-middle text-right" data-importe="<?= $detalle['cantidad']*$detalle['precio'] ?>"><?= ($detalle['cantidad']/cantidad_unidad($db,$detalle['producto_id'],$detalle['unidad_det']))*$detalle['precio'] ?></td>
										...
										<tr class="active cuotaclass">
											...
											<td>
												<div data-cuota="<?= $i ?>" class="cuota_div">
													<div class="col-sm-12">
														<input  id="inicial_fecha_<?= $i ?>" name="fecha[]" value="<?= (count($detpagos) >= $i) ? date('d-m-Y', strtotime($detpagos[$i-1]['fecha']) ) : date('d-m-Y') ?>" min="<?= date('d-m-Y') ?>" class="form-control input-sm" autocomplete="off" <?php if ($i == 1) { ?> data-validation="required date" <?php } ?> data-validation-format="DD-MM-YYYY" data-validation-min-date="<?= date('d-m-Y'); ?>" >
													</div>
												</div>
											</td>
											<td>
                ...
                <div class="panel-body">
                    <?php if ($permiso_asignar) { ?>
                        <div class="row">
                            <div class="col-xs-12 text-right">
                                <?php if ($entrega == 1) { ?>
                                    <a href="<?= back(); ?>" class="btn btn-primary"><i class="glyphicon glyphicon-arrow-left"></i><span> Atras</span></a>
                                <?php } else {?>
                                    <a href="?/asignacion/preventas_asignar/<?= $id_egreso ?>" class="btn btn-primary"><i class="glyphicon glyphicon-arrow-left"></i><span> Atras</span></a>
                                <?php } ?>
                            </div>
                        </div>
                        <hr>
								...
								<?php foreach ($productos as $nro => $producto) {
                                    ...
                                    <tr data-busqueda="<?= $producto['id_producto'] .'_'. $producto['lote']; ?>">
										...
										<td class="text-middle">
                                            ...
                                            <span class="editar hidden" data-editar="<?= $producto['id_producto'] .'_'. $producto['lote']; ?>"><?php
    ...
    function adicionar_producto(id_producto) {
		...
		if ($producto.size()) {
		    $producto.find('[data-cantidad]').removeAttr('data-validation-allowing');
            $producto.find('[data-cantidad]').removeAttr('data-validation-error-msg');
			cantidad = $.trim($cantidad.val());
			cantidad = ($.isNumeric(cantidad)) ? parseInt(cantidad) : 0;
			cantidad = (cantidad < 99999999) ? cantidad + 1 : cantidad;
            ...
            if (porciones.length > 2) {
                ...
                plantilla = plantilla + '</select></td>' +
                    ...
                    '<input type="text" value="' + $.trim(aparte[1]) + '" readonly name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="' + aparte[1] + '"   data-validation-error-msg="Debe ser un número decimal positivo" onkeyup="calcular_importe(\'' + id_producto + '\',1)">' +
                    '<input type="text" value="' + $.trim(aparte[1]) + '" readonly name="pre[]" class="form-control input-xs text-right hidden" autocomplete="off" data-pre="' + aparte[1] + '"   data-validation-error-msg="Debe ser un número decimal positivo" onkeyup="calcular_importe(\'' + id_producto + '\',1)">' +
