		...
		$tables.each(function (nro) {
				...
				if (Modernizr.datauri && Modernizr.adownload) {
					...
					$table.find('tbody tr').each(function (i) {
						...
						$(this).children().each(function (j) {
							// console.log(j);
							if (j == options.total) {
							    formateado = ($(this).text()).replace('.','');
								total = total + parseFloat(formateado); 
								// console.log('fila: ' +i + ' columna: '+j+' Total: '+total);
							}
                    ...
                    file += '<table>';
                    file += '<tr><td colspan="'+ ($table.find('thead tr:first').children().length) +'" align="Center" style="vertical-align: middle;"><h3>' +options.name+ '</h3></td></tr>';
                    file += '<tr><td align="center" colspan="'+ options.total +'" style="vertical-align: middle;"><h5>' +options.fechas+ '</h5></td>';
                    file += '<td align="center" colspan="'+ (($table.find('thead tr:first').children().length)-options.total) +'" style="vertical-align: middle;"><h5>' +options.creacion+ '</h5></td></tr>';
                    file += '</table>';
                    ...
                    file += '<table><tr><td colspan="16" align="Center" style="vertical-align: middle;"></td></tr>';
                    file += '<tr><td align="right" colspan="'+ options.total +'" style="vertical-align: middle;"><h5>Total: </h5></td>';
                    file += '<td align="left" colspan="'+ (($table.find('thead tr:first').children().length)-options.total) +'" style="vertical-align: middle;"><h5>' +parseFloat(total)+ '</h5></td></tr></table>';
					file += '</body>';
					file += '</html>';
			...
			$('#report_word_' + nro).on('click', function (e) {
				...
				// 			}
					...
					file += '<table><tr><td colspan="16" align="Center" style="vertical-align: middle;"><h3>' +options.name+ '</h3></td></tr>';
					file += '<tr><td align="center" colspan="8" style="vertical-align: middle;"><h5>' +options.fechas+ '</h5></td>';
					file += '<td align="center" colspan="8" style="vertical-align: middle;"><h5>' +options.creacion+ '</h5></td></tr>';
					file += '</table>';

					file += '<table cellspacing="0" cellpadding="0" border="1" style="font-size: 8px">' + thead + tbody + '</table>';
					
					file += '<table><tr><td colspan="16" align="Center" style="vertical-align: middle;"></td></tr>';
					file += '<tr><td align="right" colspan="13" style="vertical-align: middle;"><h5>Total: </h5></td>';
					file += '<td align="left" colspan="3" style="vertical-align: middle;"><h5>' +total.toFixed(2)+ '</h5></td></tr>';
					file += '</table>';
			...
			$('#report_pdf_' + nro).on('click', function (e) {
				...
				$table.find('thead tr:first').children().each(function (i) {
					...
					widths.push(width);
					thead.push({text: $.trim($this.text()), bold: true, fontSize: 7, fillColor: '#eee'});
				});
				...
				// 		}
						...
						if (contents[j] == 'text') {
							...
							tbody.push({text: $.trim(text), bold: false, fontSize: 6, fillColor: '#fff'});
						} else {
							text = $(this).html();
							...
							for (var k in texts) {
								...
								text = text.replace(new RegExp('(</strong>|</b>)', 'g'), '", "bold": false},');
								text = text.replace(new RegExp('(</em>|</i>)', 'g'), '", "italics": false},');
				...
				// table.push('{"text": "Total: ", "bold": true,  "fontSize": 8}', '{"text": total.toFixed(2), "bold": true,  "fontSize": 8}');
							...
							columns: [
								{
									alignment: 'left',
									text: [
										{ text: 'Total: ' + total.toFixed(2), bold: true , fontSize: 9 }
									]
								}],
								margin: [0, 10]
						}
					...
					footer: (function(page, pages) {
						return {
							columns: [
							{
								alignment: 'center',
								text: [
								{ text: page.toString(), italics: true , fontSize: 6 },
								{ text: '/', italics: true , fontSize: 6 },
								{ text: pages.toString(), italics: true , fontSize: 6 }
								]
							}],
							margin: [10, 0]
						}
