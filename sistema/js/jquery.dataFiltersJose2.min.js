...
jQuery.fn.extend({
	DataFilter: function (options) {
        ...
        //console.log(options.imag);
        //console.log(options.imag2);
		var defaults = {
			button: 'btn-default',
			filter: false,
            ordering: false,
		...
		$tables.each(function (nro) {
			...
			$fields.find(':checkbox').on('change', function () {
				...
				if (size > 0) {
					...
					tables.table(nro).column(1).visible(visible);
			...
			$('#report_excel_' + nro).on('click', function (e) {
				...
				if (Modernizr.datauri && Modernizr.adownload) {
					...
					$table.find('tbody tr').each(function (i) {
                        if (i == 0) {
                            tbody += '<tr style="border-top: 1px solid black; background-color: #bdbdbd">';
                        } else {
                            if (aqui == true) {
                                tbody += '<tr style="border-top: 1px solid black; background-color: #bdbdbd">';
                                aqui = false;
						...
						$(this).children().each(function (j) {
                            ...
                            if(j > 0) {
                                ...
                                if ($.trim(text) == 'Total') {
                                    tbody += '<td style="border: 0px solid black;">' + $.trim(text) + '</td>';
                                    aqui = true;
                                    // console.log(aqui);
                                } else {
                                    tbody += '<td style="border: 0px solid black;">' + $.trim(text) + '</td>';
					...
					file += '</head>';
					file += '<body>';
                    file += '<table><tr><td colspan="15" align="Center" style="vertical-align: middle;"><h3>' +options.name+ '</h3></td></tr><tr><td align="center" colspan="8" style="vertical-align: middle;"><h5>' +options.empresa+ '</h5></td><td align="center" colspan="8" style="vertical-align: middle;"><h5>' +options.direccion+ '</h5></td></tr></table>';
					file += '<table cellspacing="0" cellpadding="0" border="1" style="font-size: 13px">' + thead + tbody + '</table>';
					file += '</body>';
					file += '</html>';
			...
			$('#report_word_' + nro).on('click', function (e) {
				...
				if (Modernizr.datauri && Modernizr.adownload) {
					...
					file += '</head>';
					file += '<body>';
                    file += '<table width="100%" border="0" ><tr><td><img id="img-0" src="' + options.imag + '" height="50" ></td><td align="right">' +options.empresa+ '<br>' +options.direccion+ '<br>' +options.telefono+ '</td></tr></table>';
			...
			$('#report_html_' + nro).on('click', function (e) {
				...
				'<html lang="en">' +
					...
					'</head>' +
					'<body>' +
                    '<table width="100%" border="0" ><tr><td><img src="' + options.imag + '" height="50" ></td><td align="right">' +options.empresa+ '<br>' +options.direccion+ '<br>' +options.telefono+ '</td></tr></table>' +
						table +
					'</body>' +
				'</html>';
			...
			$('#report_pdf_' + nro).on('click', function (e) {
				...
				$table.find('thead tr:first').children().each(function (i) {
					if(i > 0 ){ //&& i < ($table.find('thead tr:first').children().length) 
                        var $this = $(this);
                        ...
                        widths.push(width);
                        thead.push({text: $.trim($this.text()), bold: true, fontSize: 8, fillColor: '#eee'});
                    }
				});
				...
				$table.find('tbody tr').each(function (i) {
					var tbody = [];
                    $(this).children().each(function (j) {
                        if (j > 0 ){ //&& j < ($table.find('thead tr:first').children().length)
                            if (contents[j] == 'text') {
                                text = $(this).text();
                                text = text.replace(new RegExp('\\n', 'g'), ' ');
                                text = text.replace(new RegExp('\\s+', 'g'), ' ');
                                // tbody.push($.trim(text));
                                if (i == 0 || aqui == true && punto == i) {
                                    tbody.push({text: $.trim(text), bold: true, fontSize: 6, fillColor: '#eee'});
                                } else {
                                    tbody.push({text: $.trim(text), bold: false, fontSize: 6});
                                }
                                ...
                                for (var k in texts) {
                                    ...
                                    text = text.replace(new RegExp('(</small>)', 'g'), '", "fontSize": 6},');
								...
								if (i== 0 || aqui == true && punto == i) {
									object = '{"text": [' + object + '], "bold": true, "fontSize": 6, "fillColor": "#eee"}';
								} else {
									object = '{"text": [' + object + ']}';
								}
                                // object = '{"text": [' + object + ']}';
                    ...
                    header: {
                        ...
                        columns: [
                            ...
                            },
                            {
                                stack: [
                                    {
                                        text: options.empresa,
                                        alignment: 'right',
                                        fontSize: 9
                                    },
                                    {
                                        text: options.direccion,
                                        alignment: 'right',
                                        fontSize: 8
                                    },
                                    // {
